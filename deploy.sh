...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=sampriyadarshi/multi-server:$SHA
kubectl set image deployments/client-deployment client=sampriyadarshi/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=sampriyadarshi/multi-worker:$SHA
